# Illustration Pilot Prompts (DRAFT)

작성일: 2026-04-21 (Asia/Seoul)
목적: 챕터 hero 일러스트를 "3장만 먼저" 만들어 스타일을 확정한 뒤, 나머지 챕터로 확장한다.

대상(파일명 고정):
- `chapters/images/ch00_hero.png` (프롤로그)
- `chapters/images/ch03_hero.png` (머니 스크립트)
- `chapters/images/ch06_hero.png` (인벤토리/현금흐름)

공통 스타일(요약):
- warm, minimal, friendly illustration
- 공포 마케팅/과장/부자 과시 금지
- 게임 메타포는 1개 이내로만(장식 과잉 금지)
- 텍스트는 넣지 않거나, 넣더라도 1줄 이내
- 16:9 (권장 1200x630)
...
## ch03_hero (머니 스크립트: 기억이 문제)

의도:
- "통장이 죄가 아니라, 머릿속 대본(머니 스크립트)이 문제"라는 메시지를 1장으로 요약한다.

프롬프트(초안):
- Warm, minimal illustration. A harmless-looking script page (like a theater script) is gently being edited with a pencil eraser, turning harsh red scribbles into calm notes. Next to it is a simple wallet icon and a calm heart icon, showing that feelings and money are connected. Include a subtle game metaphor: an old 'quest log' page being updated. No numbers, no charts, no fear. Soft colors, friendly and reflective.

---

## ch06_hero (인벤토리: 현금흐름/재무상태)

의도:
- "내 인벤토리를 열어 현재 좌표를 본다"를 시각화한다. 복잡한 표 대신, '정리/안심/시작' 감각을 준다.
